import { leagueRaces, leagues, teams } from "@/db/schema/leagues";
...
/**
 * Rebuild auto-generated transfer windows for a single league from its current
 * race calendar (delete existing auto windows, insert freshly generated ones).
 * Manual windows (isAutoGenerated=false) are left untouched, and transfer_bids
 * have no FK to windows, so this is safe to run any time.
 */
export async function syncTransferWindowsForLeague(
  leagueId: number,
  season: number,
) {
...
/**
 * Regenerate transfer windows for every league whose calendar includes the given
 * parent race. Call this after any race/stage/rest-day mutation so windows stay in
 * sync with the calendar without a manual "Generate windows" click. Returns the
 * number of leagues resynced. Never throws for a missing season config — such
 * leagues are skipped.
 */
export async function syncTransferWindowsForParentRace(parentRaceId: number) {
  const affected = await db
    .select({ leagueId: leagues.id, config: leagues.config })
    .from(leagueRaces)
    .innerJoin(leagues, eq(leagues.id, leagueRaces.leagueId))
    .where(eq(leagueRaces.raceId, parentRaceId));

  let resynced = 0;
  for (const row of affected) {
    const season = (row.config as { seasonYear?: number } | null)?.seasonYear;
    if (!season) continue;
    await syncTransferWindowsForLeague(row.leagueId, season);
    resynced++;
  }
  return resynced;
}
...
/**
 * Regenerate transfer windows for every league. Use after bulk calendar changes
 * (imports, parent-race deletion) where the set of affected parent races is broad
 * or a parent race may have just been removed.
 */
export async function syncAllLeaguesTransferWindows() {
  const allLeagues = await db
    .select({ id: leagues.id, config: leagues.config })
    .from(leagues);

  let resynced = 0;
  for (const row of allLeagues) {
    const season = (row.config as { seasonYear?: number } | null)?.seasonYear;
    if (!season) continue;
    await syncTransferWindowsForLeague(row.id, season);
    resynced++;
  }
  return resynced;
}
