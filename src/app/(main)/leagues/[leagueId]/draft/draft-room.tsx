...
import { makePick, refreshDraftState, skipPick } from "./actions";
...
export function DraftRoom({
  ...
  const [draftStatus, setDraftStatus] = useState<
    "pending" | "men" | "women" | "complete"
  >(initialSession.status === "paused" ? "pending" : initialSession.status);
  const [currentPickIndex, setCurrentPickIndex] = useState(
    initialSession.currentPickIndex,
  );
  const [currentTurn, setCurrentTurn] = useState<number | null>(
    initialSession.currentTeamId,
  );
  ...
  const [timerExpiresAt, setTimerExpiresAt] = useState<Date | null>(
    initialSession.timerExpiresAt
      ? new Date(initialSession.timerExpiresAt)
      : null,
  );
  const [availableMenState, setAvailableMenState] =
    useState<Rider[]>(availableMen);
  const [availableWomenState, setAvailableWomenState] =
    useState<Rider[]>(availableWomen);
  ...
  const pickedRiderMapRef = useRef<
    Map<number, { id: number; name: string; team: string }>
  >(
    ...
    new Map(
      initialPicks
        ...
        .map((p) => [
          p.riderId,
          { id: p.riderId, name: p.rider!.name, team: p.rider!.team },
        ]),
    ),
  );
  ...
  const reconnectAndSync = useCallback(async () => {
    try {
      ...
      setTimerExpiresAt(
        newSession.timerExpiresAt ? new Date(newSession.timerExpiresAt) : null,
      );
  ...
  useEffect(() => {
    ...
    channel.bind("pick-made", (data: PickMadePayload) => {
      ...
      if (pick.gender === "M") {
        setAvailableMenState((prev) =>
          prev.filter((r) => r.id !== pick.riderId),
        );
      } else {
        setAvailableWomenState((prev) =>
          prev.filter((r) => r.id !== pick.riderId),
        );
      }
      ...
      if (pickerTeam && pick.teamId !== currentTeamId) {
        toast(
          `${pickerTeam.name} made a pick${pick.wasAutomatic ? " (auto)" : ""}`,
        );
      }
    });
    ...
    async (riderId: number) => {
      ...
      try {
        ...
        if (!result.success) {
          ...
          const nextTeamId = next.isComplete
            ? null
            : (teams[next.nextTeamIndex]?.id ?? null);
          ...
          const rider = (
            currentGender === "M" ? availableMenState : availableWomenState
          ).find((r) => r.id === riderId);
          if (rider) {
            if (currentGender === "M") {
              setAvailableMenState((prev) =>
                prev.filter((r) => r.id !== riderId),
              );
            } else {
              setAvailableWomenState((prev) =>
                prev.filter((r) => r.id !== riderId),
            ...
            pickedRiderMapRef.current.set(riderId, {
              id: riderId,
              name: rider.name,
              team: rider.team,
            });
          ...
          if (result.pick) {
            setPicks((prev) => {
              if (prev.some((p) => p.pickNumber === currentPickIndex))
                return prev;
              return [
                ...prev,
                {
